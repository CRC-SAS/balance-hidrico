...
test_that("calcular_eventos_lluvia_pre_siembra cuenta dias con pp>=10mm en [-14,+7], limites inclusive", {
  ...
  expect_equal(calcular_eventos_lluvia_pre_siembra(clima_estacion, fecha_siembra), 4)
})
...
test_that("calcular_estado_hidrico_siembra selecciona la fila de fecha_siembra", {
  ...
  expect_equal(res$au_pct_m1, 0.7)
  expect_equal(res$au_pct_m2, 0.6)
  expect_equal(res$au_pct_total, 0.65)
...
test_that("calcular_confort_hidrico calcula sum(TrR)/sum(DemT) solo dentro de la ventana (limites inclusive)", {
  ...
  expect_equal(calcular_confort_hidrico(serie_diaria_balance, hitos), 15 / 50)
})
...
test_that("calcular_salidas combina las 3 salidas con la forma esperada", {
  ...
  expect_named(res, c("eventos_lluvia_pre_siembra", "estado_hidrico_siembra", "confort_hidrico"))
  expect_equal(res$eventos_lluvia_pre_siembra, 1)
  expect_equal(nrow(res$estado_hidrico_siembra), 1)
  expect_equal(res$confort_hidrico, 0.5)
})
